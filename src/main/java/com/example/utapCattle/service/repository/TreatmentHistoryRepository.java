...
public interface TreatmentHistoryRepository extends JpaRepository<TreatmentHistory, Long> {
    ...
    @Query(value = "SELECT TO_CHAR(MAX(t.withdrawaldate::date), 'DD/MM/YYYY') as withdrawalDate FROM treatmenthistory t WHERE t.cattleid = :cattleId group by t.cattleid", nativeQuery = true)
